...
class PriceUploadScreen extends StatefulWidget {
  const PriceUploadScreen({Key? key}) : super(key: key);

  @override
  _PriceUploadScreenState createState() => _PriceUploadScreenState();
}
    ...
    await _displayPickImageDialog(context!, () async {
      try {
        ...
        if (source == ImageSource.camera) {
          if (pickedFile!.path.isNotEmpty && _imageFileList!.length < 1) {
            setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Upload Price Tag'),
        actions: [
          listToUpload!.length == 1
              ? InkWell(
                  onTap: () {
